...
// ********** Code for _DeviceMessageAware **************
function _DeviceMessageAware(area) {
  this.area = area;
  if (null == $globals._DeviceMessageAware__callbacks) {
    $globals._DeviceMessageAware__callbacks = new HashMapImplementation();
    $globals._DeviceMessageAware__messageHandlers = new HashMapImplementation();
    get$$window().get$on().get$message().add(this.get$_messageDispatcher(), false);
  }
  $globals._DeviceMessageAware__messageHandlers.$setindex(this.area, this.get$receiveMessage());
}
_DeviceMessageAware.prototype.get$area = function() { return this.area; };
_DeviceMessageAware.prototype._messageDispatcher = function(event) {
  var jsonMsg = json_JSON.parse(event.data);
  print$(("got message [" + jsonMsg.$index("type") + "] target [" + jsonMsg.$index("target") + "]"));
  if ($eq$(jsonMsg.$index("target"), "Dart")) {
    var message = _DeviceMessage._DeviceMessage$json$factory(jsonMsg);
    var handler = $globals._DeviceMessageAware__messageHandlers.$index(message.get$area());
    if (null != handler) {
      var callbackId = message.get$callback();
      if (null != callbackId && $globals._DeviceMessageAware__callbacks.containsKey(callbackId)) {
        message.set$callback($globals._DeviceMessageAware__callbacks.$index(callbackId));
        $globals._DeviceMessageAware__callbacks.remove(callbackId);
      }
      handler.call$1(message);
    }
    else {
      $throw(("unhandled message area " + message.get$area()));
    }
  }
}
_DeviceMessageAware.prototype.get$_messageDispatcher = function() {
  return this._messageDispatcher.bind(this);
}
_DeviceMessageAware.prototype.sendMessage = function(message) {
  if (null != message.callback) {
    var guid = ("" + new DateImplementation.now$ctor().value + "-" + Math.random());
    $globals._DeviceMessageAware__callbacks.$setindex(guid, message.callback);
    message.callback = guid;
  }
  get$$window().postMessage(message.get$asJsonString(), "*");
}
_DeviceMessageAware.prototype.createMessage = function(type) {
  return new _DeviceMessage(this.area, type);
}
// ********** Code for _DeviceImpl **************
$inherits(_DeviceImpl, _DeviceMessageAware);
_DeviceImpl._internal$ctor = function() {
  this.notification = new _DeviceNotificationImpl();
  _DeviceMessageAware.call(this, "device");
}
_DeviceImpl._internal$ctor.prototype = _DeviceImpl.prototype;
function _DeviceImpl() {}
_DeviceImpl._DeviceImpl$factory = function() {
  if (null == $globals._DeviceImpl__instance) {
    $globals._DeviceImpl__instance = new _DeviceImpl._internal$ctor();
  }
  return $globals._DeviceImpl__instance;
}
_DeviceImpl.prototype.receiveMessage = function(message) {
  switch (message.type) {
    case "ready":

      this._onDeviceReady.call$0();
      break;
...
_DeviceImpl.prototype.get$receiveMessage = function() {
  return this.receiveMessage.bind(this);
}
_DeviceImpl.prototype.set$onDeviceReady = function(callback) {
  var $0;
  return (this._onDeviceReady = ($0 = callback), $0);
}
// ********** Code for _DeviceMessage **************
function _DeviceMessage(area, type, content) {
  this.area = area;
  this.type = type;
  this.content = content;
  if (null == this.content) {
    this.content = new HashMapImplementation();
  }
}
_DeviceMessage._DeviceMessage$json$factory = function(json) {
  return new _DeviceMessage(json.$index("area"), json.$index("type"), json.$index("content"));
}
_DeviceMessage.prototype.get$area = function() { return this.area; };
_DeviceMessage.prototype.get$callback = function() { return this.callback; };
_DeviceMessage.prototype.set$callback = function(value) { return this.callback = value; };
_DeviceMessage.prototype.get$content = function() { return this.content; };
_DeviceMessage.prototype.get$asJsonString = function() {
  var msg = _map(["area", this.area, "callback", this.callback, "content", this.content, "type", this.type, "target", "Cordova"]);
  return json_JSON.stringify(msg);
}
// ********** Code for _DeviceNotificationImpl **************
$inherits(_DeviceNotificationImpl, _DeviceMessageAware);
function _DeviceNotificationImpl() {
  _DeviceMessageAware.call(this, "notification");
}
_DeviceNotificationImpl.prototype.alert = function(alertMessage, callback) {
  var msg = this.createMessage("alert");
  msg.get$content().$setindex("alert", alertMessage);
  msg.set$callback(callback);
  this.sendMessage(msg);
}
_DeviceNotificationImpl.prototype.receiveMessage = function(message) {

}
_DeviceNotificationImpl.prototype.get$receiveMessage = function() {
  return this.receiveMessage.bind(this);
}
// ********** Code for top level **************
function get$$device() {
  return _DeviceImpl._DeviceImpl$factory();
}
//  ********** Library qalqo:dartgap:testapp **************
// ********** Code for top level **************
function main() {
  get$$device().set$onDeviceReady((function () {
    get$$document().body.get$nodes().add(_ElementFactoryProvider.Element$html$factory("<h1>Hey, it's Dart!</h1>"));
    get$$device().notification.alert("Dart said hello");
  })
  );
}
// 53 dynamic types.
